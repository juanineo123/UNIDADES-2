...
const TABLE_WIDTH_DXA = 9360; // Aprox. 6.5 pulgadas usables
...
const createSubTitle = (text) => new Paragraph({ children: [new TextRun({ text, bold: true, size: 22, font: "Calibri" })], heading: HeadingLevel.HEADING_3, spacing: { before: 200, after: 100 } });
...
const createFormattedParagraphs = (text = "", isJustified = false) => {
    ...
    const paragraphOptions = { alignment: isJustified ? AlignmentType.JUSTIFIED : AlignmentType.LEFT, spacing: { after: 100 } };
    return text.split('\n').map(line => {
...
// --- FUNCIONES CONSTRUCTORAS DE TABLAS (CON CORRECCIÓN DE REDONDEO) ---
const createCriteriaTable = (criteriaText = "") => {
    if (!criteriaText || typeof criteriaText !== 'string' || criteriaText.trim() === "") return [new Paragraph({ text: "No se especificaron criterios." })];
    ...
    return [new Table({
        ...
        rows: [
            new TableRow({ tableHeader: true, children: [new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "CRITERIOS DE EVALUACIÓN", bold: true, allCaps: true, size: 24 })] })], verticalAlign: VerticalAlign.CENTER, margins: cellMargins })] }),
            ...criteria.map(criterion => new TableRow({ children: [new TableCell({ children: createFormattedParagraphs(criterion.replace(/^- |^\* |^• /, '').trim()), verticalAlign: VerticalAlign.CENTER, margins: cellMargins })] })),
        ],
    })];
};
...
const createTableFromMarkdown = (markdownText = "") => {
    ...
    // --- CORRECCIÓN FINAL: Se usa Math.floor para asegurar número entero ---
    ...
    return new Table({
        ...
        rows: [
            new TableRow({ tableHeader: true, children: headerCells.map(headerText => new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: headerText, bold: true, allCaps: true, size: 22 })] })], verticalAlign: VerticalAlign.CENTER, margins: cellMargins })) }),
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const createBulletedList = (items) => (items && items.length > 0) ? items.map(item => new Paragraph({ text: item, bullet: { level: 0 } })) : [new Paragraph({ text: "" })];
        ...
        const doc = new Document({
            ...
            sections: [{
                ...
                children: [
                    ...
                    new Table({
                        width: { size: TABLE_WIDTH_DXA, type: WidthType.DXA },
                        // --- CORRECCIÓN FINAL: Se usa Math.floor para asegurar números enteros ---
                    ...
                    new Table({
                        width: { size: TABLE_WIDTH_DXA, type: WidthType.DXA },
                         // --- CORRECCIÓN FINAL: Se usa Math.floor para asegurar números enteros ---
                        columnWidths: [Math.floor(TABLE_WIDTH_DXA * 0.25), Math.floor(TABLE_WIDTH_DXA * 0.30), Math.floor(TABLE_WIDTH_DXA * 0.45)],
                        rows: [
                            new TableRow({ tableHeader: true, children: [new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "COMPETENCIA", bold: true, allCaps: true })] })], verticalAlign: VerticalAlign.CENTER }), new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "CAPACIDADES", bold: true, allCaps: true })] })], verticalAlign: VerticalAlign.CENTER }), new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "DESEMPEÑOS PRECISADOS", bold: true, allCaps: true })] })], verticalAlign: VerticalAlign.CENTER })] }),
                            new TableRow({ children: [new TableCell({ children: [new Paragraph(competencia.nombre || 'No especificada')], verticalAlign: VerticalAlign.CENTER }), new TableCell({ children: createBulletedList(competencia.capacidades ? competencia.capacidades.map(c => c.nombre) : []), verticalAlign: VerticalAlign.CENTER }), new TableCell({ children: createBulletedList(competencia.capacidades ? competencia.capacidades.flatMap(c => c.desempenos || []) : []), verticalAlign: VerticalAlign.CENTER })] }),
                        ],
                    }),
                    ...
                    ...createFormattedParagraphs(generatedContent.producto, true),
                    createSectionTitle("IV. SECUENCIA DIDÁCTICA"),
                    new Table({
                        width: { size: TABLE_WIDTH_DXA, type: WidthType.DXA },
                        // --- CORRECCIÓN FINAL: Se usa Math.floor para asegurar números enteros ---
                        columnWidths: [Math.floor(TABLE_WIDTH_DXA * 0.2), Math.floor(TABLE_WIDTH_DXA * 0.8)],
                        rows: [
                            new TableRow({ tableHeader: true, children: [new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "MOMENTOS", bold: true, allCaps: true })] })] }), new TableCell({ verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "PROCESOS PEDAGÓGICOS Y ACTIVIDADES", bold: true, allCaps: true })] })] })] }),
